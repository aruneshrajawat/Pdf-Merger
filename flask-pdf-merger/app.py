...
def process_merge():
    ...
    try:
        ...
        print(f"DEBUG: Redirecting to download page for: {output_filename}")
...
def create_test_file():
    ...
    try:
        ...
        # Copy sample1.pdf to output folder as test
        sample_path = os.path.join('sample_pdfs', 'sample1.pdf')
        ...
        if os.path.exists(sample_path):
            ...
            flash('Test file created successfully!', 'success')
            return redirect(url_for('download', filename=filename))
        else:
            flash('Sample PDF not found', 'error')
            return redirect(url_for('index'))
        
    except Exception as e:
        flash(f'Failed to create test file: {str(e)}', 'error')
        return redirect(url_for('index'))
...
def test_download_direct():
    """Direct test of download functionality"""
    filename = 'test_download.pdf'
    file_path = os.path.join(app.config['OUTPUT_FOLDER'], filename)
    ...
    if os.path.exists(file_path):
        ...
        return "Test file not found. Run: python3 simple_download_test.py", 404
